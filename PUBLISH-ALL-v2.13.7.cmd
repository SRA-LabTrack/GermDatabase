...
title CaneSprout v2.13.7 - Build + GitHub + Vercel Auto Deploy

set "REPO=https://github.com/SRA-LabTrack/GermDatabase.git"
set "MSG=CaneSprout v2.13.7 web login rebalanced showcase boxes to avoid overlap on web and electron"

echo.
echo ============================================================
echo   CANESPROUT v2.13.7 PUBLISH
...
if not "%APPVER%"=="2.13.7" (
  echo [ERROR] This publish script expects CaneSprout v2.13.7.
...
if errorlevel 1 (
  ...
  git merge origin/main --allow-unrelated-histories -s ours -m "Merge GitHub history before CaneSprout v2.13.7" || goto :fail
) else (
  git branch -M main || goto :fail
...
echo.
echo ============================================================
echo   SUCCESS - CANESPROUT v2.13.7 PUSHED
echo ============================================================
echo GitHub: %REPO%
echo Vercel: Git integration will deploy main automatically.
echo Electron installer: release\CaneSprout-Registry-Setup-2.13.7.exe
echo.
