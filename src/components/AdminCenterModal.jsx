...
export default function AdminCenterModal({ currentUser, initialTab = 'approvals', onClose, onRegistryChanged, online = navigator.onLine }) {
  ...
  return <div className="modal-backdrop"><section className="modal admin-center-modal">
    ...
    <div className="modal-content admin-content">{!online ? <div className="admin-empty"><ShieldCheck size={30} /><h3>Admin Center is cached and ready</h3><p>Registry edits, imports, deletions and combinations can be queued offline. Approval decisions and account creation require an online server session, so CaneSprout will not send futile Appwrite/Vercel requests while this device is offline.</p></div> : tab === 'approvals' ? <ApprovalsTab currentUser={currentUser} onRegistryChanged={onRegistryChanged} /> : <AccountsTab currentUser={currentUser} />}</div>
  </section></div>;
}
