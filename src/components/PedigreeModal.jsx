...
export default function PedigreeModal({ onClose, onOpenProfile, initialVariety = '' }) {
  ...
  useEffect(() => {
    if (loading || !initialVariety || !catalog.length) return;
    const initialKey = normalizeVarietyIdentity(initialVariety);
    const initialRecord = catalog.find((record) => normalizeVarietyIdentity(record.variety) === initialKey);
    if (!initialRecord) {
      setQuery(normalizeVarietyDisplay(initialVariety));
      return;
    }
    setSelected(initialRecord);
    setQuery(normalizeVarietyDisplay(initialRecord.variety || initialVariety));
  }, [loading, initialVariety, catalog]);
      ...
      <section className="pedigree-modal-shell">
        <header className="pedigree-modal-header">
          <div>
            ...
            <h2 id="pedigree-title">Germplasm Pedigree</h2>
