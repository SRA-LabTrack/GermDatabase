...
export default function SpreadsheetEditorModal({ onClose, onSaved, actor = null, online = navigator.onLine }) {
  ...
  async function applyChanges() {
    ...
    try {
      if (!online || !navigator.onLine) {
        ...
        for (const item of changes) {
          await queueOfflineRecord({
            ...
            form: item.record,
            recordId: item.record?.__bundledSnapshot ? '' : item.record?.$id || '',
            previous: item.previous || null,
            operationOverride: item.record?.__bundledSnapshot || item.previous?.__bundledSnapshot ? 'upsert' : ''
          });
          queued += 1;
          setProgress(`Saving ${queued}/${changes.length} changed row${changes.length === 1 ? '' : 's'} to this device…`);
        }
        const nextOriginals = { ...originals };
        changes.forEach((item) => { nextOriginals[item.record.$id] = cloneRecord(item.record); });
        setOriginals(nextOriginals);
        setChanged(new Set());
        setNotice(`${queued} changed row${queued === 1 ? '' : 's'} saved offline. CaneSprout will sync them gradually when the connection returns.`);
        onSaved?.({ offline: true, queued });
        return;
      }
      ...
      if (result.errors.length) {
        const ownerId = actor?.id || actor?.$id || actor?.email || 'local-user';
        const changesById = new Map(changes.map((item) => [item.record?.$id, item]));
        let queued = 0;
        const remainingErrors = [];
        for (const issue of result.errors) {
          const networkish = !navigator.onLine || /network|fetch|unreachable|timed out|timeout/i.test(String(issue?.message || ''));
          const item = changesById.get(issue.id);
          if (!networkish || !item) {
            remainingErrors.push(issue);
            continue;
          }
          try {
            await queueOfflineRecord({
        ...
        const finishedIds = new Set(result.saved.map((record) => record.$id));
        if (queued) {
          result.errors.forEach((issue) => {
            if (!remainingErrors.includes(issue)) finishedIds.add(issue.id);
          });
          setNotice(`${result.saved.length} row${result.saved.length === 1 ? '' : 's'} reached Appwrite and ${queued} interrupted row${queued === 1 ? '' : 's'} were saved offline.`);
          onSaved?.({ offline: true, queued });
        }
        setChanged((current) => new Set([...current].filter((id) => !finishedIds.has(id))));
        if (remainingErrors.length) setError(`${remainingErrors.length} row${remainingErrors.length === 1 ? '' : 's'} still need attention. First error: ${remainingErrors[0].message}`);
      } else {
        const nextOriginals = { ...originals };
  ...
  return <div className="modal-backdrop spreadsheet-backdrop">
    <section className="modal spreadsheet-editor-modal">
      ...
      <div className="spreadsheet-controls">
        <div className="spreadsheet-note"><SugarcaneIcon size={21} /><div><strong>Edit cells, then apply once.</strong><span>Nothing is written while you type. Apply changes writes to Appwrite when online or to the local offline queue when disconnected.</span></div></div>
