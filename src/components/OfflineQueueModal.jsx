    ...
    try {
      const result = await syncOfflineQueue({
        ...
        onProgress: (event) => {
          ...
          if (event.phase === 'delete') setProgress(`Deleting ${name} from Appwrite…`);
    ...
    <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section className="modal offline-queue-modal">
              ...
              {entries.map((entry) => (
                <article key={entry.id} className={`offline-entry ${entry.status === 'error' ? 'has-error' : ''}`}>
                  <div className="offline-entry-main">
                    ...
                    <div><small>{entry.operation === 'create' ? 'New field record' : entry.operation === 'delete' || entry.operation === 'deleteByVariety' ? 'Queued deletion' : entry.operation === 'upsert' ? 'Queued offline upsert' : 'Queued edit'} • {ageText(entry.createdAt)}</small><h3>{entry.form?.variety || entry.form?.germ_trial_code || 'Unnamed sugarcane record'}</h3><p>{entry.form?.germ_location || 'Location not recorded'}{entry.photos?.length ? ` • ${entry.photos.length} compressed photo${entry.photos.length === 1 ? '' : 's'}` : ' • no new photos'}</p>{entry.lastError && <span className="offline-entry-error">{entry.lastError}</span>}</div>
                  </div>
        ...
        <footer className="modal-footer">
          ...
          {!!entries.length && <button className="primary-button" onClick={() => sync()} disabled={busy || !online}>{busy ? <><LoaderCircle className="spin" size={16} /> Syncing…</> : <><CloudUpload size={16} /> Sync next 20</>}</button>}
        </footer>
      </section>
    </div>
  );
