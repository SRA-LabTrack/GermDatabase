...
const OFFLINE_CURSOR_PREFIX = 'offline-offset:';
...
function offlineOffset(cursor) {
  if (!String(cursor || '').startsWith(OFFLINE_CURSOR_PREFIX)) return 0;
  const value = Number(String(cursor).slice(OFFLINE_CURSOR_PREFIX.length));
  return Number.isFinite(value) && value >= 0 ? Math.floor(value) : 0;
}

async function offlineWorkspacePage({ search = '', scope = 'variety', cursor = '', limit = PAGE_SIZE, recent = false } = {}) {
  const normalizedScope = normalizeScope(scope);
  let snapshots = [];
  try { snapshots = await listOfflineRecords(); } catch {}

  // A live record previously seen on this device wins over the bundled baseline
  // with the same canonical variety identity. This preserves the real Appwrite
  // document ID for offline edits while the audited bundled registry fills any
  // gaps without a cloud read.
  const liveByIdentity = new Map();
  snapshots.forEach((record) => {
    const key = normalizeVarietyIdentity(record?.variety || '');
    if (key) liveByIdentity.set(key, record);
  });
  const merged = [];
  const seenIds = new Set();
  BUNDLED_RECORDS.forEach((record) => {
    const key = normalizeVarietyIdentity(record?.variety || '');
    const replacement = key ? liveByIdentity.get(key) : null;
    const value = replacement ? { ...record, ...replacement, $id: replacement.$id, __bundledSnapshot: false, __offlineSnapshot: true } : record;
    if (!seenIds.has(value.$id)) {
      seenIds.add(value.$id);
      merged.push(value);
    }
    if (replacement) liveByIdentity.delete(key);
  });
  liveByIdentity.forEach((record) => {
  ...
  const filtered = merged
    ...
    .sort((a, b) => bundledText(a.variety).localeCompare(bundledText(b.variety)));
  const source = recent ? filtered.slice(0, RECENT_LIMIT) : filtered;
  const offset = recent ? 0 : offlineOffset(cursor);
  const documents = source.slice(offset, offset + limit);
  const nextOffset = offset + documents.length;
  const hasMore = !recent && nextOffset < source.length;
  return {
    documents,
    nextCursor: hasMore ? `${OFFLINE_CURSOR_PREFIX}${nextOffset}` : '',
    hasMore,
    skippedForShortSearch: false,
    matchMode: recent ? 'offline-recent' : (search ? 'offline-search' : 'offline-browse'),
    bundledSnapshot: snapshots.length === 0,
    offlineWorkspace: true,
    offlineFallback: true,
    bundledTotal: filtered.length,
    cachedLiveCount: snapshots.length,
    fromCache: true
  };
}
...
export async function listRecentRecords({ bypassCache = false } = {}) {
  if (typeof navigator !== 'undefined' && navigator.onLine === false) return offlineWorkspacePage({ recent: true, limit: RECENT_LIMIT });
  ...
  const execute = async () => {
    try {
      ...
      cacheOfflineRecords(documents).catch(() => {});
      memoryCache.set(key, { savedAt: Date.now(), value });
      writeSessionCache(key, value);
      return { ...value, fromCache: false };
    } catch (error) {
      if (isNetworkFailure(error)) return offlineWorkspacePage({ recent: true, limit: RECENT_LIMIT });
      throw error;
    }
  };
...
export async function listRecords({ search = '', scope = 'variety', cursor = '', strategy = 'auto', bypassCache = false } = {}) {
  ...
  if (String(cursor || '').startsWith(OFFLINE_CURSOR_PREFIX)) {
    return offlineWorkspacePage({ search: term, scope: normalizedScope, cursor });
  }
  ...
  const execute = async () => {
    try {
      ...
      cacheOfflineRecords(value.documents).catch(() => {});
      memoryCache.set(key, { savedAt: Date.now(), value });
      writeSessionCache(key, value);
...
export async function listSpreadsheetRecords({ cursor = '', search = '', scope = 'variety' } = {}) {
  if (typeof navigator !== 'undefined' && navigator.onLine === false) {
    const result = await offlineWorkspacePage({ search, scope, cursor, limit: SHEET_PAGE_SIZE });
    const documents = [];
    for (const record of result.documents || []) {
      if (record.__bundledSnapshot) documents.push({ ...OPTIONAL_TRAIT_DEFAULTS, ...record });
      else {
        const full = await getOfflineRecord(record.$id).catch(() => null);
        documents.push(full || record);
      }
    }
...
export async function exportAllRecords(onProgress) {
  if (typeof navigator !== 'undefined' && navigator.onLine === false) {
    const page = await offlineWorkspacePage({ limit: Math.max(5000, BUNDLED_RECORDS.length + 500) });
    onProgress?.({ stage: 'offline', pages: 0, records: page.documents.length, detailPages: 0, detailRecords: page.documents.length });
    return Promise.all((page.documents || []).map(async (record) => {
      if (record.__bundledSnapshot) return { ...OPTIONAL_TRAIT_DEFAULTS, ...record };
      const full = await getOfflineRecord(record.$id).catch(() => null);
      const baseline = BUNDLED_IDENTITY_MAP.get(normalizeVarietyIdentity(record.variety || '')) || {};
      return { ...OPTIONAL_TRAIT_DEFAULTS, ...baseline, ...(full || record), $id: record.$id };
    }));
  }
...
export async function getLocalRecordPreview(record) {
  ...
  return {
    ...
    ...(snapshot || {}),
    ...record,
    $id: record.$id || snapshot?.$id || baseline?.$id,
...
export async function getRecord(recordId, { bypassCache = false } = {}) {
  if (String(recordId || '').startsWith(BUNDLED_RECORD_ID_PREFIX)) {
    ...
    // If this variety was previously seen live, prefer its persisted Appwrite
    // identity and full cached payload so offline edits target the real record.
    const live = await findOfflineRecordByVariety(snapshot.variety).catch(() => null);
    return live ? { ...OPTIONAL_TRAIT_DEFAULTS, ...snapshot, ...live, $id: live.$id, __bundledSnapshot: false, __offlineSnapshot: true } : { ...OPTIONAL_TRAIT_DEFAULTS, ...snapshot };
  }
  if (typeof navigator !== 'undefined' && navigator.onLine === false) {
    const offline = await getOfflineRecord(recordId).catch(() => null);
    if (offline) {
      const baseline = BUNDLED_IDENTITY_MAP.get(normalizeVarietyIdentity(offline.variety || '')) || {};
      return { ...OPTIONAL_TRAIT_DEFAULTS, ...baseline, ...offline, $id: offline.$id, __bundledSnapshot: false, __offlineSnapshot: true };
    }
  }
  ...
  if (exists) {
    try {
      return await withAppwriteFailover(() => databases.updateDocument({
        ...
        documentId,
        data
      }), { retryTransport: false, timeoutMs: 12000 });
    } catch (error) {
...
export async function deleteRecordByVariety(variety) {
  ...
  if (matches.length > 1) throw new Error(`Cannot delete “${display}” automatically because ${matches.length} live records normalize to that variety.`);
  const record = await getRecord(matches[0].$id, { bypassCache: true });
  await deleteRecord(record);
  return { deleted: true, recordId: record.$id, variety: record.variety || display };
}
...
/**
 * Create a record from the manual Add Record form.
 *
 * Manual creation deliberately uses the same canonical identity index used by
 * Excel import. That keeps duplicate behavior consistent while avoiding the
 * older double-path where the form could fail before reaching the actual
 * create call. A caller-supplied ID makes retries idempotent after an uncertain
 * network response.
 */
export async function createManualRecord(data, recordId = '', onProgress) {
  ...
  const identityIndex = await loadCanonicalIdentityIndex({ force: true });
  const matches = [];
  const seen = new Set();
  for (const key of varietyIdentityKeys(variety)) {
    for (const document of identityIndex.get(key) || []) {
      if (recordId && document.$id === recordId) continue;
  ...
  if (matches.length) {
    const examples = matches.slice(0, 3).map((item) => item.variety || item.$id).join(', ');
    throw new Error(`Duplicate variety blocked: “${variety}” already matches ${matches.length} registry record${matches.length === 1 ? '' : 's'} (${examples}).`);
  }

  onProgress?.('Registering the new variety in Appwrite…');
  const saved = await saveRecord(
    { ...data, variety, source_name: data?.source_name || 'Manual entry' },
    recordId || ID.unique(),
    null,
    { knownNew: true, skipDuplicateGuard: true }
  );

  // Keep the duplicate guard warm so an immediate second manual create sees
  // the newly registered variety without another full identity scan.
  if (identityGuardCache.index) addCoreToIdentityIndex(identityGuardCache.index, saved);
  return saved;
}
...
export async function upsertRecordByVariety(data, previousHint = null, { clearBlankCells = false } = {}) {
  ...
  if (!matches.length) {
    const created = await saveRecord({ ...data, variety, source_name: data?.source_name || 'Offline sync' }, '', null, { knownNew: true, skipDuplicateGuard: true });
    addCoreToIdentityIndex(identityIndex, created);
    identityGuardCache = { savedAt: Date.now(), index: identityIndex };
    return { record: created, created: true, updated: false };
  }
  if (matches.length > 1) {
    throw new Error(`Duplicate conflict: ${matches.length} existing records normalize to “${variety}”. Resolve the duplicate conflict before syncing this offline change.`);
  }
  const core = matches[0];
  const previous = previousHint?.$id === core.$id && !previousHint?.__bundledSnapshot
    ? previousHint
    : await getRecord(core.$id, { bypassCache: true });
  const merged = mergeImportedValues(previous, data, { clearBlankCells });
  const saved = await saveRecord(merged, core.$id, previous);
  addCoreToIdentityIndex(identityIndex, saved);
  identityGuardCache = { savedAt: Date.now(), index: identityIndex };
