...
export function resolveBreedingReportRange(request = {}) {
  ...
  const legacyMonth = /^\d{4}-\d{2}$/.test(text(request.month)) ? text(request.month) : '';
  const explicitYear = clampReportYear(
    request.monthYear,
    legacyMonth ? Number(legacyMonth.slice(0, 4)) : Number(currentIsoDate().slice(0, 4))
  );
  const explicitMonth = Math.max(
    1,
    Math.min(
      12,
      Number(request.monthNumber)
        || (legacyMonth ? Number(legacyMonth.slice(5, 7)) : Number(currentIsoDate().slice(5, 7)))
    )
  );
  const safeMonth = `${explicitYear}-${pad2(explicitMonth)}`;
  const monthYear = explicitYear;
