...
console.log(`\nCaneSprout ${pkg.version} Smooth Germplasm Map Redirect Verification\n`);

check(Boolean(pkg.scripts?.['verify:variety-map-smooth-redirect']), 'Smooth redirect verifier npm script exists');

check(component.includes('map.stop()'), 'A new redirect stops any previous map animation');
check(component.includes('duration: 0.72'), 'Nearby redirects use slower 0.72s movement');
...
check(component.includes("'moveend'"), 'Popup waits for movement to finish');
check(component.includes('popupMoveEndRef'), 'Stale movement popup callbacks are tracked/cancelled');
...
check(css.includes('will-change: transform'), 'Tile compositing receives transform hint');
check(css.includes('backface-visibility: hidden'), 'Tile compositing reduces transition flicker');
check(css.includes('.leaflet-zoom-animated'), 'Animated Leaflet layers receive transform compositing hint');
