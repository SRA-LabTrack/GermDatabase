...
const combinationsCollection = 'sugarcane_combination_registry';
...
const combinationAttrs = [
  textAttr('male_variety', 255),
  textAttr('male_key', 255),
  textAttr('female_variety', 255),
  textAttr('female_key', 255),
  textAttr('combination_date', 32),
  textAttr('notes', 1000),
  textAttr('created_by', 36),
  textAttr('created_by_name', 128),
  textAttr('created_at', 32),
  textAttr('search_text', 1024)
];
...
async function main() {
  console.log('\nSugarcane Germplasm Resource Database setup v2.10.0');
  ...
  await ensure(`collection ${combinationsCollection}`,
    `/databases/${databaseId}/collections/${combinationsCollection}`,
    `/databases/${databaseId}/collections`,
    { collectionId: combinationsCollection, name: 'SUGARCANE_COMBINATION_REGISTRY', permissions: combinationPermissions, documentSecurity: false, enabled: true });
  await ensureAttributes(combinationsCollection, combinationAttrs);
  ...
  await waitForAttributes(combinationsCollection, combinationAttrs.map((attribute) => attribute.key));
  ...
  await enforceCollectionPolicy(combinationsCollection, 'SUGARCANE_COMBINATION_REGISTRY', combinationPermissions, false);
  ...
  await ensureIndex(combinationsCollection, 'idx_combo_male', 'key', ['male_key'], ['ASC']);
  await ensureIndex(combinationsCollection, 'idx_combo_female', 'key', ['female_key'], ['ASC']);
  await ensureIndex(combinationsCollection, 'idx_combo_date', 'key', ['combination_date'], ['DESC']);
  await ensureIndex(combinationsCollection, 'fts_combo_search', 'fulltext', ['search_text']);
  ...
  await waitForIndexes(combinationsCollection, ['idx_combo_male', 'idx_combo_female', 'idx_combo_date', 'fts_combo_search']);
  ...
  console.log('• Combination Registry stores dated male × female breeding history in sugarcane_combination_registry.');
