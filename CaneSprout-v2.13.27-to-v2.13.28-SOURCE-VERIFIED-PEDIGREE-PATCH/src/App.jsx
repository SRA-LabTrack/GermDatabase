    ...
    <main className="app-shell">
      <header ref={toolbarRef} className={`topbar reference-toolbar ${isAdmin ? 'admin-toolbar' : 'user-toolbar'} ${desktopMode ? 'electron-toolbar' : 'web-pedigree-toolbar'}`}>
        ...
        {mobileToolbarOpen && (
          <section id="mobile-toolbar-menu" className="mobile-toolbar-menu" aria-label="CaneSprout tools menu">
            ...
            <div className="mobile-toolbar-menu-list">
              ...
              {!desktopMode && (
                <button type="button" onClick={() => { setMobileToolbarOpen(false); setShowExcelMenu(false); setShowForm(false); setEditRecord(null); setShowAdminCenter(false); setShowOfflineQueue(false); setShowImport(false); setShowExportExcel(false); setShowSpreadsheetEditor(false); setShowCombinationRegistry(false); setShowPedigree(true); }}>
                  <GitBranch size={20} />
        ...
        <nav className={`toolbar-main-actions segmented-toolbar ${isAdmin ? 'admin-actions' : ''}`} aria-label="Primary registry navigation">
          <button
            className={`toolbar-tile nav-button ${!showImport && !showExcelMenu && !showExportExcel && !showSpreadsheetEditor && !showForm && !showAdminCenter && !showCombinationRegistry && !showPedigree ? 'active' : ''}`}
            aria-current={!showImport && !showExcelMenu && !showExportExcel && !showSpreadsheetEditor && !showForm && !showAdminCenter && !showCombinationRegistry && !showPedigree ? 'page' : undefined}
          ...
          {!desktopMode && (
            <button
              ...
              onClick={() => { setShowExcelMenu(false); setShowForm(false); setEditRecord(null); setShowAdminCenter(false); setShowOfflineQueue(false); setShowImport(false); setShowExportExcel(false); setShowSpreadsheetEditor(false); setShowCombinationRegistry(false); setShowPedigree(true); }}
      ...
      <Suspense fallback={<ModalLoading />}>
        ...
        {showPedigree && !desktopMode && <PedigreeModal onClose={() => setShowPedigree(false)} onOpenProfile={(recordId) => { setShowPedigree(false); setDetailId(recordId); }} />}
      </Suspense>
    </main>
  );
}
